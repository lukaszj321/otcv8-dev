...
**Cel:** Jednolita, operacyjna specyfikacja **parsera/serializera OTUI (STRICT)**, **walidatora** i **macierzy dozwolonych dzieci**. Dokument jest fundamentem dla edytora TS (Sparky) oraz testĂłw round-trip.
...
## # 0.1 Zakres {: #ch-0-1 }
- Pokrycie: *caĹ‚y pipeline* od tekstu OTUI (STRICT) â†” AST (TS) â†” walidacja â†” auto-naprawy â†” eksport/import.  
...
- **DeterministycznoĹ›Ä‡**: ten sam AST â†’ ten sam OTUI (bit-identyczny, przy tej samej wersji serializera).  
...
> **Legenda:** âś“ dozwolone Â· âś– zabronione Â· ďż˝  warunkowe (patrz uwagi).
## # 4.1 Okna (Window-class) {: #ch-4-1 }
...
| TextList | âś“ | Wymaga pary z VerticalScrollBar przy overflow (ďż˝ ).
| ProgressBar, HorizontalSeparator | âś“ | â€”
| TabBar, TabWidget, Splitter | âś“ | Splitter: dokĹ‚adnie 2 dzieci (ďż˝ ).
| StatusOverlay | âś“ | Warstwa wierzchnia, bez zĹ‚oĹĽonych dzieci (ďż˝ ).
| *Window (Main/Static/Mini/Container/Dialog) | âś– | Zakaz okien-dzieci.
...
- Slot `content`: elementy panelowe âś“; okna-dzieci âś–.  
...
- JeĹ›li sÄ… `anchors.left` + `anchors.right` **i** `width` â†’ usuĹ„ `width` (Auto-fit).  
...
<div id="ch-7"></div>
## 7. Import/Export i round-trip (edytor â†” plik â†” Lua)
...
- Opcjonalnie: *round-trip do Lua* â€” odĹ›wieĹĽ istniejÄ…cy blok `[[...]]` bit-identycznie po `ensureStrictOtui()`.
...
**Interaktywny spis:** [9.1 Goldeny round-trip](#ch-9-1) Â· [9.2 Snapshoty wizualne](#ch-9-2)
## # 9.1 Goldeny round-trip {: #ch-9-1 }
- Zestaw `X.otui` â†’ `parse` â†’ `serialize` â†’ porĂłwnanie bit-po-bicie.  
...
<div id="ch-11"></div>
## 11. Indeks haseĹ‚
- STRICT â€˘ AST â€˘ Macierz â€˘ Walidator â€˘ Auto-naprawy â€˘ Round-trip â€˘ Splitter â€˘ TabBar/TabWidget â€˘ StatusOverlay â€˘ Scroll pairing
