# Specyfikacja implementacji (MASTER v1.0): **Studio React/Electron** dla skrypt�w **OTClient v8** + **modules/game_bot (vBot)**

> Dokument centralny - pelna, profesjonalna specyfikacja do autonomicznej implementacji narzedzia. Zawiera architekture, modele danych, protokoly, checklisty, reguly jakosci, plan wdrozenia, testy, ryzyka i artefakty startowe. Wszystkie kroki sa deterministyczne i mozliwe do zautomatyzowania na podstawie danych zawartych w tym dokumencie.

---
# # 0. Executive Summary
- **Cel:** zbudowac wieloplatformowe Studio (desktop **Electron** + **React/TypeScript**) do tworzenia, analizy i utrzymania skrypt�w **Lua/OTUI/OTML** dla **OTClient v8** oraz **vBot**.
- **Core features:**
  1) Inteligentne **podpowiedzi API** (kuratorowany `api.json` + indeks ze skanu repo),
  2) **Lint/auto-fix** dla **OTUI** i lekkie reguly dla **Lua**,
  3) **Generator** modul�w/szablon�w,
...
- **Hot-reload** - `g_modules.reloadModules()` (skr�t w kliencie lub modul dev).
- **Flag file** - plik-flaga `modules/.dev/reload.flag` wyzwalajacy hot-reload.
- **NDJSON** - log *newline-delimited JSON* (kazda linia = obiekt JSON).

---
# # 2. Zakres / Poza zakresem
**MVP w zakresie:** edycja Lua/OTUI/OTML, podpowiedzi API, lint/auto-fix (OTUI + Lua-lite), generator szablon�w, skan repo i indeks, Log Viewer, hot-reload.

**Poza MVP:** step-debugger (breakpointy), zdalne eval Lua; profiling runtime; zlozone RPC do klienta (inne niz plik/log/skr�t).

---
# # 3. Architektura systemu
**Wariant rekomendowany:** Desktop **Electron** (React/TS + Node/FS), integracja z OTClient przez system plik�w i skr�t hot-reload.
...
**FR-01 - Projekty:** wyb�r katalogu, ostatnie projekty, walidacja (`modules/`, `.otmod`).
...
**FR-05 - Lint OTUI:** kolejnosc p�l, `tr()` dla stalych, anchors/margins, istnienie zasob�w; auto-fix dla kolejnosci i `tr()`.
**FR-06 - Lint Lua (lite):** preferuj `local`, wykryj globalne; `table.unpack` vs `unpack`.
**FR-07 - Generator:** moduly (`.otmod` + `main.lua` + `ui/*.otui`) + snippety vBot z checklista.
**FR-08 - Skan/Indeks:** glob, parsery Lua/OTUI/OTML, relacje `dofile/require` i `g_ui.loadUI`; cache inkrementalny.
**FR-09 - Integracja z OTClient:** przycisk "Przeladuj w kliencie" (skr�t/flag file).
**FR-10 - Log Viewer:** tail NDJSON/tekst, filtry (level/tag/file:line), wyszukiwarka, eksport.
**FR-11 - Ustawienia:** sciezki artefakt�w i logu, ignore patterns, motyw; import/eksport.
**FR-12 - Wbudowany help:** panel "API & How-to", wyszukiwarka offline.

**Kryteria akceptacji (wybrane):** indeks 5k plik�w < 5 s z cache; brak crashy przy edycji plik�w 2 MB; >90% trafien podpowiedzi dla znanych symboli; auto-fix deterministyczny.
...
> Kazdy JSON zawiera `$schemaVersion` i jest walidowany testami kontraktowymi.
...
```json
{
  ...
  "functions": [
    {"name": "g_modules.reloadModules", "module": "g_modules", "params": [], "returns": [], "description": "Przeladowuje moduly i skrypty OTClient.", "since": "v8"},
  ...
  "events": [
    {"name": "onGameStart", "target": "Game", "payload": [], "description": "Wywolywane przy starcie gry."}
],
  "objects": [
    {"name": "g_resources", "members": ["readFile", "writeFile"], "description": "Menedzer zasob�w."}
]
}
```
> **Uwaga:** to seed. Pelny `api.json` powstaje wg �7 z lokalnych plik�w dokumentacji.
...
```json
{"$schemaVersion":1,"rules":[{"id":"OTUI-001","description":"Kolejnosc p�l: GEOMETRIA?STYL?ZACHOWANIE.","fixable":true},{"id":"OTUI-002","description":"Stale stringi musza uzywac tr().","fixable":true},{"id":"OTUI-003","description":"Walidacja anchors/margins (brak sprzecznosci).","fixable":false},{"id":"OTUI-004","description":"Weryfikacja istnienia zasob�w (obrazy, fonty, style).","fixable":false}]}
```
# # # 6.4. `templates.json` (generator)
```json
{"$schemaVersion":1,"module.default":{"title":"Modul OTClient - szkielet","files":[{"path":"modules/hello/hello.otmod","contents":"name: hello\n"},{"path":"modules/hello/hello.lua","contents":"-- entry point\nlocal M = {}\nreturn M\n"},{"path":"modules/hello/ui/hello.otui","contents":"MainWindow < UIWidget { }\n"}],"checklist":["Umiesc katalog w modules/","Uruchom klienta i przeladuj moduly","Sprawdz log po starcie modulu"]}}
```
# # # 6.5. `docstrings.json`
```json
...
**Wejscia:** lokalne pliki dokumentacji (`OTClient_data_documentation_FULL.md`, `luafunctionscpp.md`, `modules_Documentation.md`, `src framework.md`, `src client.md`).

**Kroki:**
1) Segmentacja MD (H1-H3) i klasyfikacja sekcji: API/Events/Hooks/Globals/Managers.
2) Ekstrakcja sygnatur: `name(params) -> returns`, opis�w, przyklad�w, statusu (`since/deprecated`).
3) Normalizacja nazewnictwa (`module`: `g_resources`, `g_modules`, itp.).
4) Walidacja: unikalne `name`, kompletne `params/returns`.
5) Scalanie z `docstrings.json` (uzupelnienia typ�w/przyklad�w).
6) Eksport `resources/api.json` (dodaj `generatedAt`, `$schemaVersion`).

**Wzorce (regex - przykladowe):**
- Funkcja: `^\s*([A-Za-z0-9_\.\:]+)\s*\(([^)]*)\)\s*(?:->\s*([^\n{]+))?`
- Param: `---@param\s+(\w+)\s+([\w<>\|]+)`
- Return: `---@return\s+([\w<>\|]+)`
- Event: `\bon[A-Z][A-Za-z0-9]+\b` (lub sekcje "Events").

**Polityka typ�w:** `string|number|boolean|table|any|nil|function|userdata`; `optional` jesli oznaczone `?` lub w kwadratowych nawiasach.

**Quality Gate:** 0 duplikat�w; ?95% pozycji z opisem i kompletnymi parametrami; walidacja JSON wg schematu.
...
**Parser Lua:**
- Tokenizacja: identyfikatory, stringi, komentarze (`--`, blokowe), slowa kluczowe.
...
```
- Kategoryzacja atrybut�w: GEOMETRIA (`x`,`y`,`width`,`height`,`anchors`,`margin`), STYL (`font`,`color`,`image`,`style`), ZACHOWANIE (`draggable`,`onClick`,`id`).
- Relacje: `g_ui.loadUI("path")` ? `relations.lua_to_otui`.

**Parser `.otmod`:** klucz:wartosc (+ listy); wymagane `name`; waliduj znaki w `name`.

**Kody bled�w parsera:** `P001` (Lua tokenizer), `P101` (OTUI niezamkniety blok), `P201` (.otmod brak `name`).

---
# # 9. Lint/Auto-fix - reguly
- **OTUI-001 (order):** sortuj atrybuty: GEOMETRIA ? STYL ? ZACHOWANIE (auto-fix: przetasuj z zachowaniem komentarzy).
- **OTUI-002 (tr):** stale literaly string wrapuj `tr()` (auto-fix; ignoruj `id`/nazwy klas).
- **OTUI-003 (anchors):** wykryj sprzecznosci anchors/margins (sugestie, bez auto-fix).
- **OTUI-004 (assets):** zglos brakujace zasoby (fuzzy podpowiedzi).
- **LUA-001 (locals):** ostrzegaj globali; auto-fix: `local` jesli bez kolizji.
- **LUA-002 (unpack):** zamieniaj `unpack` -> `table.unpack` (auto-fix bezpieczny).

---
# # 10. Integracja z OTClient (hot-reload, logi)
**Tryb A (podstawowy):** zapis ? skr�t hot-reload w kliencie (np. Ctrl+Shift+R).

**Tryb B (z modulem dev):** Studio zapisuje `modules/.dev/reload.flag`; modul dev w kliencie wykrywa i:
1) wywoluje `g_modules.reloadModules()`,
2) usuwa/przepisuje plik flagi,
3) opcjonalnie loguje wynik (NDJSON) do `modules/.dev/log.jsonl`.

**Szkic modulu dev (Lua):**
```lua
local dev = {}
...
---
# # 11. UI/UX (ekrany i przeplywy)
- **Start:** wyb�r/ostatnie projekty, skr�ty do dokumentacji, "Nowy modul".
- **Eksplorator:** drzewo plik�w, akcje kontekstowe, status operacji FS.
- **Edytor:** Monaco + panel boczny (API/Doc), statusbar (pozycja, problems, encoding).
- **Diagnostyka:** lista Issues (filtry), Quick Fix, przejscia do linii.
- **API Browser:** lista obiekt�w ? szczeg�ly (opis, params, examples) ? "Wstaw snippet".
- **Log Viewer:** stream z filtrami (ERR/WARN/INFO), szukanie, pauza, eksport.
- **Ustawienia:** sciezki zasob�w, logu, motyw, ignorowane foldery, mapowanie skr�tu.
...
**Bledy:** `E_FS_001` (dostep FS), `E_PARSE_LUA`, `E_PARSE_OTUI`, `E_JSON_SCHEMA`.
...
---
# # 14. Bezpieczenstwo i prywatnosc
- Renderer sandbox, restrykcyjne IPC, brak sieci (domyslnie), brak wykonywania Lua.
- Backup `.bak` przy auto-fixach; zapisy atomowe.

---
# # 15. Wydajnosc i stabilnosc
- Worker Threads dla parser�w; debounce 150 ms; cache w `.studio-cache/`.
- "Big file mode" w Monaco > 2 MB (ograniczenie czesci funkcji).
...
- **Integration:** skan 5k plik�w, pomiary czasu/RAM.
...
- Auto-update (opcjonalny), domyslnie wylaczony - narzedzie offline-first.

---
# # 19. Observability narzedzia
- Log aplikacji: `.studio/logs/app.ndjson`; poziomy: DEBUG/INFO/WARN/ERROR.
- Metryki: czas skanu, liczba plik�w, czas lintu, bledy parser�w.

---
# # 20. Migracje i zgodnosc
- `$schemaVersion` w artefaktach; migracje `vN -> vN+1` dla config/cache.

---
# # 21. Plan wdrozenia i checklisty
...
- [ ] Parser OTUI/OTML + kategoryzacja atrybut�w.
- [ ] Parser `.otmod`.
...
- [ ] Przeladuj (skr�t/flag) + Log Viewer.

**Etap 5 - Stabilizacja**
- [ ] Testy (unit/contract/integration/E2E), pakiety instalacyjne.

**Definition of Done (Koncowe):**
- [ ] Aplikacja offline; projekty sie otwieraja; indeks i symbole dzialaja.
...
- [ ] Pakiety Win/macOS/Linux; dokumentacja uzytkownika; sample project.

---
# # 22. Ryzyka i mitigacje
- **Brak step-debuggera:** Log Viewer + snippety logujace + mapowanie file:line.
- **R�znice wersji:** `since/deprecated` w `api.json`, profile klienta.
- **Wydajnosc:** indeks inkrementalny, workers, throttling.
...
---
# # 24. Przykladowe dane (seed do startu)
**A. `resources/api.json` (seed)** - patrz �6.1 (pelny JSON wstawiony).

**B. `resources/otui-rules.json` (seed)** - patrz �6.3 (pelny JSON wstawiony).

**C. `resources/templates.json` (seed)** - patrz �6.4 (pelny JSON wstawiony).

---
# # 25. Noty koncowe
- Ten dokument jest **zr�dlem prawdy (SoT)**. Dodatkowe canvasy musza odwolywac sie do sekcji (�) i kontrakt�w tutaj zdefiniowanych.
- Zmiany wymagaja podniesienia `$schemaVersion` i przejscia test�w kontraktowych.
- Wszelkie dane sa przygotowane z mysla o pracy **offline** w oparciu o lokalne pliki dokumentacji i kodu.
