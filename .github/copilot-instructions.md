...
3) Bloki `diff` stosuj tylko dla celów pod `docs/authoring/**`.
4) Brak ścieżki ⇒ brak zapisu (log „unmapped code block”).
**Wyjątek:** Generatory C++/Lua/OTUI mogą pisać bezpośrednio do `docs/authoring/**` na podstawie źródeł objętych `applyTo`.
...
## D) Datasets
- Ścieżki: `docs/authoring/datasets/{api,ui,modules}/**.{csv,ndjson}`
...
## F) Kryteria PR
- Zmiany wyłącznie w `docs/authoring/**`.
